...
async function loadPromptMeta(promptId) {
  try {
    ...
    console.warn(`Warning: Could not load meta.json for ${promptId}:`, error.message)
    return null
  }
}
...
async function generateIndex() {
  ...
  const indexData = {
    ...
    difficulty_levels: difficultyCounts,
    featured_prompts: config.featured_prompts,
    prompts: promptsData.map(meta => ({
      ...
      featured: config.featured_prompts.includes(meta.id),
      tags: meta.tags || [],
  ...
  console.log(`⭐ Featured prompts: ${config.featured_prompts.length}`)
  console.log(`📂 Categories: ${updatedCategories.map(c => `${c.title} (${c.count})`).join(', ')}`)
}
